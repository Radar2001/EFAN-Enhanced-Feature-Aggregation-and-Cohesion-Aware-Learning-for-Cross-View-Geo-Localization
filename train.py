...
from sample4geo.dataset.university_new import U1652DatasetEval, U1652DatasetTrain, get_transforms
from sample4geo.utils import setup_system, Logger
from sample4geo.trainer import train
from sample4geo.evaluate.university import evaluate
from sample4geo.loss_EFAN import InfoNCE, InfoNCE_inter,InfoNCE_interself, CircleLossWithSmoothing,AdaptiveHybridLoss
# from sample4geo.model import TimmModel
from model.EFAN_ViT_new_l1 import TimmModel_u


@dataclass
class Configuration:
    net: str = 'EFAN_ViT_new_l1_02_noquery_proj_AdaptiveHybridLoss'
    # Model
    ...
    verbose: bool = True
    gpu_ids: tuple = (1,2)  # GPU ids for training
...
if __name__ == '__main__':
    ...
    loss_function = AdaptiveHybridLoss(loss_function=loss_fn,
    ...
    # loss_function = loss_function + l1_penalty
    ...
    if config.decay_exclue_bias:
        ...
        # 假设你的模型类有一个参数叫 self.global_scale_logit 或 self.global_scale
        # scale_param = [model.global_scale_logit]   # 或 model.global_scale
        # other_params = [p for p in model.parameters() if p is not scale_param]
        # optimizer = torch.optim.AdamW([
        #     {'params': other_params, 'lr': config.lr},
        #     {'params': [scale_param], 'lr': config.lr * 50, 'weight_decay': 0.0}
        # ])
        scale_params = []
        other_params = []
        for name, param in model.named_parameters():
            if 'global_scale_logit' in name:  # 请确认实际名称
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if hasattr(loss_function, 'set_epoch'):
            loss_function.set_epoch(epoch - 1)  # 注意：如果你希望第1轮epoch=0
