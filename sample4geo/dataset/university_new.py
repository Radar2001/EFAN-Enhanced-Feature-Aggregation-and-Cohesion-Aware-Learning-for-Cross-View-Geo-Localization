...
from sample4geo.weather import MultiWeather
...
def get_transforms(img_size,
    ...
    train_drone_ops = [A.ImageCompression(quality_range=(90,100), p=0.5),
                       A.Resize(img_size[0], img_size[1], interpolation=cv2.INTER_AREA, p=1.0),
                       A.ColorJitter(brightness=0.15, contrast=0.15, saturation=0.15, hue=0.15, p=0.5),
                       A.OneOf([
                           A.AdvancedBlur(p=1.0),
                       ...
                       A.OneOf([
                           A.GridDropout(ratio=0.4, p=1.0),
                           A.CoarseDropout(num_holes_range=(10, 25),
                                           hole_height_range=(int(0.1 * img_size[0]),
                                                              int(0.2 * img_size[0])),
                                           hole_width_range=(int(0.1 * img_size[1]),
                                                             int(0.2 * img_size[1])),
                                           p=1.0),
                       ], p=0.3)]

    if weather:
        train_drone_ops.append(MultiWeather(weather_idx=weather_idx, p=1.0))

    train_drone_ops += [A.Normalize(mean, std), ToTensorV2()]

    train_drone_transforms = A.Compose(train_drone_ops)

    return val_transforms, train_sat_transforms, train_drone_transforms


def get_weather_query_val_transforms(img_size,
                                     mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225],
                                     weather_idx=None):
    """用于测试阶段对 drone query 图像施加固定/随机天气的 transforms。"""
    return A.Compose([A.Resize(img_size[0], img_size[1], interpolation=cv2.INTER_AREA, p=1.0),
                      MultiWeather(weather_idx=weather_idx, p=1.0),
                      A.Normalize(mean, std),
                      ToTensorV2(),
                      ])